...
#define CODE_L9X    55                  /* obs code: SB+C     L1/L5    (IRN) */
...
#define IONOOPT_IF12 2                  /* ionosphere option: L1/L2 oriono-free LC */
...
typedef struct {
	...
	int ion_const;          //使用外电离层积的限制constraint of using extern ionospheric product, 0:off, 1:on
...
typedef struct {
	...
	int delEp;              //当前纪元和前一纪元之间的纪元数the number of epochs between the current epoch and previous epoch
	double sample;
...
typedef struct {        /* satellite status type */
	...
	double resc_pri[NFREQ]; /*载波相位残差 residuals of carrier-phase (m) */
