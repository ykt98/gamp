...
#define MAX_ITER    8               /*最大迭代次数 max number of iterations */
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	for (f=0;f<NF(&rtk->opt);f++) {
		/* 如果obs中断计数器过期，重置相位偏差reset phase-bias if expire obs outage counter */
		for (i=0;i<MAXSAT;i++) {
		...
		/* 纠正相位码跳变，确保相位码一致性correct phase-code jump to ensure phase-code coherency */
	...
	double *v, double *H, double *R, double *azel)
{
	...
	for (i=0;i<n&&i<MAXOBS;i++) {
		...
		//基于GIM的电离层延迟ionospheric delay derived from GIM
		if (PPP_Glo.prcOpt_Ex.ion_const) {
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	//calculate satellite elevation计算卫星高度
	calElev(rtk,obs,n,rs);

	//for modeling ionosphere variations between epochs用于模拟不同时期电离层的变化
	/* initialize position for first epoch */
	if (norm(rtk->x,3)<=0.0)
	...
	//to compute the elements of initialized PPP files计算初始化的PPP文件的元素
	if (PPP_Glo.outFp[OFILE_IPPP]) {
		...
		for (i=0;i<n&&i<MAXOBS;i++) {
			...
			//line-of-sight (LOS) unit vector视线单位矢量
	...
	/*ekf状态的时间更新 temporal update of ekf states */
